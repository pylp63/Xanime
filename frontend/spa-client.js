...
(function initNavAuth() {
  ...
  // Theme toggle — injected first, works regardless of login state, survives
  // innerHTML swaps below because we delegate the click on the container.
  const toggle = '<button id="theme-toggle" title="切换 黑/白 主题" aria-label="切换主题" class="btn-glass" style="padding:0.5rem 0.7rem; line-height:0;">' +
  ...
  function applyThemeBtn() {
    ...
    nav.insertAdjacentHTML('afterbegin', toggle);
  }
  applyThemeBtn();
  ...
  nav.addEventListener('click', function (e) {
    ...
    if (themeBtn) {
      ...
      try { localStorage.setItem('xanime-theme', next); } catch (err) {}
      return;
    }
  ...
  fetch('/api/me', { credentials: 'same-origin' })
    ...
    .then(function (me) {
      ...
      nav.innerHTML =
        '<button id="theme-toggle" title="切换 黑/白 主题" aria-label="切换主题" class="btn-glass" style="padding:0.5rem 0.7rem; line-height:0;">' +
          '<svg class="icon-moon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>' +
          '<svg class="icon-sun" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"/></svg>' +
        '</button>' +
...
async function renderDetail(id) {
  ...
  if (!xanime) {
    app.innerHTML = '<div class="text-center py-20"><p class="text-text-muted text-lg">影片未找到</p><a href="/" class="brand-text mt-4 inline-block">返回首页</a></div>';
    return;
  }
      ...
      '<div class="flex flex-col md:flex-row gap-8 mb-10">' +
        ...
        '<div class="glass rounded-3xl p-6 flex-1">' +
          ...
          '<div class="flex flex-wrap items-center gap-3 text-sm mb-4">' +
            (xanime.rating > 0 ? '<span class="star">★ ' + xanime.rating.toFixed(1) + '</span>' : '') +
            '<span class="text-text-muted">' + xanime.year + '</span>' +
            '<span class="tag-chip">' + xanime.category + '</span>' +
